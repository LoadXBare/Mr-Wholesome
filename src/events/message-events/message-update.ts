...
export const messageUpdate = async (oldMessage: Message | PartialMessage, newMessage: Message | PartialMessage) => {
	...
	const { author, channel, guildId, member } = newMessage;
	const maxLength = 800;

	const logChannel = await fetchLogChannel(guildId, newMessage.client);
	...
	if (oldContent.length > maxLength)
		oldContent = `${oldContent.slice(0, maxLength)}...`;
	...
	const logEntry = new MessageEmbed()
		...
		.setFields([
			{ name: 'Channel', value: `${channelMention(channel.id)} ${hyperlink('Jump to message', newMessage.url)}` },
